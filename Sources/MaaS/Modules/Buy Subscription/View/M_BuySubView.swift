...
class M_BuySubView: UIView {
    ...
    struct ViewState {
        ...
        static let initial = ViewState(state: [], dataState: .loaded, linkCardCommand: nil, onOffer: nil)
    }
    ...
    @IBOutlet weak var offerPublicLabel: GradientLabel!
    ...
    override func awakeFromNib() {
        ...
        offerPublicLabel.gradientColors = [UIColor.from(hex: "#4AC7FA").cgColor, UIColor.from(hex: "#E649F5").cgColor]
        offerTitleLabel.font = Appearance.getFont(.offer)
        offerPublicLabel.font = Appearance.getFont(.offer)
    ...
    private func hide() {
        [tableView, payButton, offerTitleLabel, offerPublicLabel].forEach { $0?.isHidden = true }
    }
    
    private func show() {
        [tableView, payButton, offerTitleLabel, offerPublicLabel].forEach { $0?.isHidden = false }
    }
    
    @objc private func handleOffer() {
        viewState.onOffer?.perform(with: ())
    }
    ...
    private func render() {
        ...
        case .loaded:
            ...
            let tapGesture = UITapGestureRecognizer(target: self, action: #selector(handleOffer))
            offerPublicLabel.addGestureRecognizer(tapGesture)
